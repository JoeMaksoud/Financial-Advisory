...
def render_profile(profile, time_filter):
    ...
    if profile["id"] == "trump":
        ...
        # ── apply BOTH time filter and sentiment filter ────────────────────
        # Date cutoff logic based on global time_filter
        ...
        def parse_mention_date(date_str):
            """Parse 'May 7, 2026' → datetime. Returns None if unparseable."""
            try:
        ...
        cutoffs = {
            "Last week":      datetime(2026, 5, 10) - timedelta(weeks=1),
            "Last month":     datetime(2026, 5, 10) - timedelta(days=30),
            "Last 6 months":  datetime(2026, 5, 10) - timedelta(days=180),
        }
        cutoff = cutoffs.get(time_filter, cutoffs["Last 6 months"])

        mentions = []
        for m in TRUMP_MENTIONS:
            # time filter
            d = parse_mention_date(m["date"])
            if d and d < cutoff:
                continue
            # sentiment filter
            if sent_filter and sent_filter != "All":
                if sent_filter == "Positive" and "Positive" not in m["sentiment"]:
                    continue
                if sent_filter == "Negative" and "Negative" not in m["sentiment"]:
                    continue
                if sent_filter == "Mixed" and m["sentiment"] != "Mixed":
                    continue
            mentions.append(m)

        # ── summary strip (reflects current filter) ───────────────────────
        pos_count = sum(1 for m in mentions if "Positive" in m["sentiment"])
        neg_count = sum(1 for m in mentions if "Negative" in m["sentiment"])
        mix_count = sum(1 for m in mentions if m["sentiment"] == "Mixed")

        period_label = time_filter or "Last 6 months"
        mc1, mc2, mc3, mc4 = st.columns(4)
        mc1.metric("Mentions in period", len(mentions))
        mc2.metric("Positive 🚀", pos_count)
        mc3.metric("Negative ⚠️", neg_count)
        mc4.metric("Mixed ⚡", mix_count)

        if not mentions:
            st.info(f"No brand mentions found for '{period_label}'"
                    + (f" with sentiment '{sent_filter}'" if sent_filter and sent_filter != "All" else "")
                    + ". Try a longer time period or different sentiment filter.")
        else:
